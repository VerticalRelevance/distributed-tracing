...
class CallTracer:
    ...
    def trace_calls(self, file_path: str, function_name: str, indent: int = 0) -> dict:
        ...
        # Initialize the node for this function
        node = {
            "name": qualified_name,
            "file": file_path,
            "type": "function",
        ...
        if "." in function_name:
            ...
            if is_inherited and source_class:
                ...
                node["type"] = "method"
                node["inherited"] = True
                node["inherited_from"] = self.get_qualified_name(
        ...
        if not function_def:
            node["status"] = "definition_not_found"
            return node
        ...
        for call in calls:
            ...
            if call in self.builtin_functions:
                ...
                call_node = {"name": call, "type": "builtin_function"}
                node["calls"].append(call_node)
                continue
                ...
                if call_file:
                    ...
                    if source_obj:
                        ...
                        if package_name and package_name != source_obj:
                            if obj_class:
                                call_node = {
                                    "name": f"{package_name}.{obj_class}.{call}",
                                    "type": "external_method",
                                    "source": (
                                        library_info if library_info else package_name
                                    ),
                                }
                            else:
                                call_node = {
                                    "name": f"{package_name}.{source_obj}.{call}",
                                    "type": "external_method",
                                }
                        else:
                            if obj_class:
                                call_node = {
                                    "name": f"{obj_class}.{call}",
                                    "type": "external_method",
                                    "source": library_info if library_info else None,
                                }
                            else:
                                call_node = {
                                    "name": f"{source_obj}.{call}",
    ...
    def generate_call_tree(self, file_path: str, function_name: str) -> dict:
        """Generate a JSON call tree starting from the specified function."""
        self.visited_calls = set()  # Reset visited calls
        return self.trace_calls(file_path, function_name)


def main():
    parser = argparse.ArgumentParser(
        description="Trace function calls in Python source files and output as JSON"
    )
    ...
    tracer = CallTracer(os.path.dirname(source_file))

    # Generate call tree
    call_tree = tracer.generate_call_tree(source_file, function_name)
    ...
    result = {
        "metadata": {
            ...
            "base_directory": tracer.base_directory,
            "traced_calls_count": len(tracer.visited_calls),
    ...
    if args.output:
        ...
        print(f"Call tree written to {args.output}")
    else:
...
if __name__ == "__main__":
    """
    python python_function_call_tracer_v11.py path/to/source.py function_name
    # For methods:
    python python_function_call_tracer_v11.py path/to/source.py ClassName.method_name
    # To save output to a file:
    python python_function_call_tracer_v11.py path/to/source.py function_name -o output.json
    """
    main()
