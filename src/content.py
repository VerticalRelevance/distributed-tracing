...
class DataMeshClient:
    ...
    def grant_tag_resource(
        self, lakeformation, principal_id, tag_key, tag_values, permissions
    ):
        return lakeformation.grant_permissions(
            Principal={"DataLakePrincipalIdentifier": principal_id},
            Resource={"LFTag": {"TagKey": tag_key, "TagValues": tag_values}},
            Permissions=permissions,
            PermissionsWithGrantOption=permissions,
        )

    def grant_tag_policy(
        self,
        lakeformation,
        principal_id,
        tag_key,
        ...
        logging.debug(
            f"tag key in policy grant {tag_key} for permissions {permissions} on resource type {resource_type} "
        )
        try:
            return lakeformation.grant_permissions(
                Principal={"DataLakePrincipalIdentifier": principal_id},
            ...
            if e.response["Error"]["Message"].lower() == "tag key does not exist":
                ...
                available_tags = lakeformation.list_lf_tags(
    ...
    def grant_tbac_to_account(
        self, lakeformation, account_id, account_type, tag_key, tag_values
    ):
        """
        ...
        if account_type == "producer":
            ...
            raise Exception(
                " invalid account type: {account_type}, can be 'producer' or 'consumer'"
            )
        logging.debug(
            f" Granting tag policy {data_permissions} to {account_id} on {tag_key} : {tag_values}"
        )
        resp = self.grant_tag_policy(
            lakeformation,
        ...
        logging.debug(
            f"tag policy response status for table policy : {resp['ResponseMetadata']['HTTPStatusCode']}"
        )
        ...
        logging.debug(
            f"tag policy response status for database policy : {resp['ResponseMetadata']['HTTPStatusCode']}"
        )
        ...
        logging.debug(
            f" Granting tag resource permissions {tag_permissions} to {account_id} on tag_key : {tag_key}, tag_values : {tag_values}"
        )
        self.grant_tag_resource(
            lakeformation,
            principal_id=account_id,
            tag_key=tag_key,
            tag_values=tag_values,
        ...
        logging.debug(
            f"tag resource response status : {resp['ResponseMetadata']['HTTPStatusCode']}"
        )
    ...
    def catalog_table_names(self, glue, dbname):
        ...
        try:
            return [
                x["Name"] for x in glue.get_tables(DatabaseName=dbname)["TableList"]
            ]
        except glue.exceptions.EntityNotFoundException:
            logging.error(
                f" Glue Database not found. Try running deploy-all.sh before this script "
            )
            return []
    ...
    def build_account_id_map(cls, sessions):
        ...
        for profile, _ in cls.PROFILES:
            ...
            acct_id_map[profile] = session.client("sts").get_caller_identity()[
                "Account"
            ]
        return acct_id_map
    ...
    def update_lf_tag(self, lakeformation, tag_key, tag_values):
        ...
        if tagvalues_to_add:
            return lakeformation.update_lf_tag(
                TagKey=tag_key, TagValuesToAdd=tagvalues_to_add
            )
    ...
    def tag_database(lakeformation, database_name, tag_key, tag_values):
        logging.debug(
            f" Tagging database {database_name} with key {tag_key} values {tag_values} "
        )
        ...
        logging.debug(
            f" tag database response status code : {resp['ResponseMetadata']['HTTPStatusCode']}"
        )
        return resp
    ...
    def tag_table(lakeformation, table_name, database_name, tag_key, tag_values):
        logging.debug(
            f" Tagging rable {database_name} with key {tag_key} values {tag_values} "
        )
        resp = lakeformation.add_lf_tags_to_resource(
    ...
    def central_glue_tasks(self):
        ...
        for central_table_name in self.catalog_table_names(glue, central_db_name):
            ...
            logging.debug(
                f" creating consumer resource link for {central_table_name} to {consumer_table_name} "
            )
            self.create_resource_link(
                consumer_glue,
            ...
            logging.debug(
                f" tagging table {central_table_name} in consumer account with {tag_key} : {tag_values} "
            )
            self.tag_table(
                consumer_lakeformation,
    ...
    def parse():
        ...
        parser.add_argument(
            ...
            "--profile",
            type=str,
            choices=["producer", "consumer", "central"],
            dest="profile",
        )
